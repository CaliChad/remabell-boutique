...
export function calculateCartTotalKobo(cart) {
    ...
    // Import discount config dynamically to avoid circular deps
    ...
    try {
        ...
        // discount module not available, no discount
    }

    return cart.reduce((total, item) => {
        const priceInKobo = parsePrice(item.price);
        const discountKobo = discountActive ? (discountAmount * 100) : 0;
        const discountedPriceKobo = Math.max(0, priceInKobo - discountKobo);
        return total + (discountedPriceKobo * item.quantity);
    }, 0);
}

/**
 * Calculate total discount amount in kobo
 * @param {Array} cart - Cart items array
 * @returns {number} Total discount in kobo
 */
export function calculateDiscountKobo(cart) {
    if (!cart || cart.length === 0) return 0;
    ...
    return totalItems * discountAmount * 100;
}
